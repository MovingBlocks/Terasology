...
/**
 * The world of Blockmania. At its most basic the world contains chunks consisting of blocks
 * and the player.
 *
 * The world is randomly generated by using one or more perlin noise generators initialized
...
public class World extends RenderableObject {
    ...
    public void render() {
         ...
         */
        glPushMatrix();
        glTranslatef(_player.getPosition().x, Configuration.VIEWING_DISTANCE_IN_CHUNKS.y * Chunk.CHUNK_DIMENSIONS.y * 1.25f, Configuration.VIEWING_DISTANCE_IN_CHUNKS.z * Chunk.CHUNK_DIMENSIONS.z + _player.getPosition().z);
        glDisable(GL_FOG);
    ...
    private int calcBlockPosX(int x1, int x2) {
        x1 = x1 % ((int) Configuration.VIEWING_DISTANCE_IN_CHUNKS.x * (int) Chunk.CHUNK_DIMENSIONS.x);
        return (x1 - (x2 * (int) Chunk.CHUNK_DIMENSIONS.x));
    }

    private int calcBlockPosY(int y1, int y2) {
        y1 = y1 % ((int) Configuration.VIEWING_DISTANCE_IN_CHUNKS.y * (int) Chunk.CHUNK_DIMENSIONS.y);
        return (y1 - (y2 * (int) Chunk.CHUNK_DIMENSIONS.y));
    }

    private int calcBlockPosZ(int z1, int z2) {
        z1 = z1 % ((int) Configuration.VIEWING_DISTANCE_IN_CHUNKS.z * (int) Chunk.CHUNK_DIMENSIONS.z);
