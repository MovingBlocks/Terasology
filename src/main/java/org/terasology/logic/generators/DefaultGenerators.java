...
public class DefaultGenerators {
    ...
    public DefaultGenerators(ForestGenerator mngr) {
        ...
        TreeGenerator oakTree = new TreeGeneratorLSystem("FFFFFFA", rules, 4, 30).setLeafType(blockManager.getBlock("engine:GreenLeaf"))
        ...
        TreeGenerator pineTree = new TreeGeneratorLSystem("FFFFAFFFFFFFAFFFFA", rules, 4, 35).setLeafType(blockManager.getBlock("engine:DarkLeaf"))
        ...
        TreeGenerator birkTree = new TreeGeneratorLSystem("FFFFAFFFFBFFFFAFFFFBFFFFAFFFFBFF", rules, 4, 35).setLeafType(blockManager.getBlock("engine:DarkLeaf"))
        ...
        TreeGenerator oakVariationTree = new TreeGeneratorLSystem("FFFFFFA", rules, 4, 35).setLeafType(blockManager.getBlock("engine:GreenLeaf"))
                .setBarkType(blockManager.getBlock("engine:OakTrunk")).setGenerationProbability(0.08f);

        // A red tree
        rules = ImmutableMap.<Character, LSystemRule>builder()
                .put('A', new LSystemRule("[&FFAFF]////[&FFAFF]////[&FFAFF]", 1.0f)).build();
        TreeGenerator redTree = new TreeGeneratorLSystem("FFFFFAFAFAF", rules, 4, 40).setLeafType(blockManager.getBlock("engine:RedLeaf"))
