...
public class EntityTools {
    ...
    public EntityTools(WorldProvider worldProvider1,EntityManager entityManager1){
    	this.entityManager=entityManager1;//CoreRegistry.get(EntityManager.class);
    	this.worldProvider=worldProvider1;
    }
     ...
     */
    public EntityRef NearestEntity(Vector3f position){
    	EntityRef nearest=entityManager.create();
     ...
     * @return Nearest entity or NULL if none
     */
    public EntityRef NearestEntityWhitComponent(Vector3f position,Class<? extends Component> componentClass){
    	EntityRef nearest=entityManager.create();
    	float lengthSquared=Float.MAX_VALUE;
     ...
     * @return
     */
    public Map<Float, EntityRef>  EntitysInRange(Vector3f position, float range){
     ...
     * @return
     */
    public Map<Float, EntityRef> EntitysInRangeWhitComponent(Vector3f position, float range, Class<? extends Component> componentClass){    	
    	Map<Float, EntityRef> tree= new TreeMap<Float, EntityRef>();
    	for (EntityRef entity : entityManager.iteratorEntities(componentClass,LocationComponent.class)) {
    		LocationComponent loc = entity.getComponent(LocationComponent.class);
    		Vector3f dist = new Vector3f(position);
    ...
    /**
     * return first entity whit given component
     * @param componentClass
     * @return entity whit component
     */
    public EntityRef firstEntityWhitComponent(Class<? extends Component> componentClass){
    	for (EntityRef entity : entityManager.iteratorEntities(componentClass,LocationComponent.class)) {
    			return entity;
    	}
     ...
     * @return
     */
    public EntityRef firstEntityWhitComponentInRange(Vector3f position,float range,Class<? extends Component> componentClass){
    	for (EntityRef entity : entityManager.iteratorEntities(componentClass,LocationComponent.class)) {
    		LocationComponent loc = entity.getComponent(LocationComponent.class);
    		Vector3f dist = new Vector3f(position);
    		dist.sub(loc.getWorldPosition());
    ...
    public boolean isPlayerInRange(Vector3f position,float range){
    	for (EntityRef entity : entityManager.iteratorEntities(LocationComponent.class)) {
			if ((entity.hasComponent(LocalPlayerComponent.class) || entity.hasComponent(PlayerComponent.class))
					&& entity.hasComponent(LocationComponent.class)) {
    ...
    /**
     * Test if surrounding blocks are penetrable
     * starts given level upward
     * @param pos position
     * @param x dimension of checked area on x 
     * @param y dimension of checked area on y
     * @param z dimension of checked area on z
     * @return if surrounding is penetrable
     */
    public boolean isSuroundingPenetrable(Vector3f position,int x,int y,int z){
    ...
    /**
     * Test if surrounding blocks are equal to given block
     * starts given level upward
     * @param pos position
     * @param x dimension of checked area on x 
     * @param y dimension of checked area on y
     * @param z dimension of checked area on z
     * @param Block block to be compared to
     * @return if surrounding are given block
     */
    public boolean isSurounding(Vector3f position,int x,int y,int z,Block block){
    	if(!testVariablesSurounding(position))
    		return false;
    	int h=x/2;
    	int w=z/2;
    ...
    /**
     * Test if surrounding blocks are equal to given block's name
     * starts given level upward
     * @param pos position
     * @param x dimension of checked area on x 
     * @param y dimension of checked area on y
     * @param z dimension of checked area on z
     * @param String name of block to be compared to
     * @return if surrounding are given block
     */
    public boolean isSurounding(Vector3f position,int x,int y,int z,String name){
    	if(!testVariablesSurounding(position))
    		return false;
    	int h=x/2;
    	int w=z/2;
