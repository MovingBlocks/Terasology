...
public class LocalWorldProvider extends PersistableObject implements IWorldProvider {
    ...
    /* CONST */
    protected final long DAY_NIGHT_LENGTH_IN_MS = new Long((Integer) ConfigurationManager.getInstance().getServerSetting("World.DiurnalCycle.dayNightLengthInMs"));
    protected final Vector2f SPAWN_ORIGIN = new Vector2f(
            (Integer)ConfigurationManager.getInstance().getServerSetting("World.Creation.spawnOriginX"),
            (Integer)ConfigurationManager.getInstance().getServerSetting("World.Creation.spawnOriginY")
    );

    /* PROPERTIES */
    protected String _title, _seed;
    protected long _creationTime = Terasology.getInstance().getTime() - new Long((Integer) ConfigurationManager.getInstance().getServerSetting("World.DiurnalCycle.initialTimeOffsetInMs"));
