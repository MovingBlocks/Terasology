...
public class ClasspathResourceLoader {
    ...
    private scanJar(String path) {
        ...
        {
            log.fine("We're not running inside a jar file, so we don't need any references to jar resources");
        }
    }
    ...
    public getClassesAt(String subPath) {
        ...
        log.fine("Getting classes from " + subPath)
    ...
    private getInternalImages(String path) {
        ...
        URL u = getClass().getClassLoader().getResource(path);
        log.fine("*** Going to get Images from classpath: " + path)
        ...
        new File(u.toURI()).list().each { i ->
            ...
            if (i.endsWith(".png")) {
                log.fine(Level.FINE, "Useful image: " + i)
    ...
    private getInternalImagesFromJar(String path) {
        ...
        while (entries.hasMoreElements()) {
            ...
            if (name.startsWith(path)) { // We only care about stuff under the desired path
                ...
                if (entry[-1] == '/') {
                    log.fine(Level.FINE, "This one is a dir, ignoring it")
                } else {
                    // We check to see if any item adds return false, meaning the item already existed (bad)
                    if (!result.add(entry)) {
                        log.fine(Level.FINE, "Hit a dupe image - this may not be bad but killing everything anyway just in case!")
        ...
        result.each {
            log.fine(it + " is being loaded as an image and mapped to short name " + it[(it.lastIndexOf('/') + 1)..-5])
            // Load a BufferedImage and put it in the map tied to its name short the ".png"
