...
/**
 * The abstract class that all serializers derive from. It by default provides the ability to serialize/deserialize an object to/from a
 * {@link PersistedData} using the given {@link PersistedDataSerializer}.
 * <p>
 * Implementors simply need to specify the type of {@link PersistedDataSerializer} to use and can provide convenience methods that use
 * {@link #serializeToPersisted(Object, TypeInfo)} and {@link #deserializeFromPersisted(PersistedData, TypeInfo)}.
 */
public final class Serializer<D extends PersistedData> {
    ...
    /**
     * Serializes the given object to a {@link PersistedData} using the stored {@link #persistedDataSerializer} by loading a {@link
     * org.terasology.persistence.typeHandling.TypeHandler TypeHandler} from the {@link #typeHandlerLibrary}.
     *
     * @param object The object to serialize.
     * @param typeInfo A {@link TypeInfo} specifying the type of the object to serialize.
     * @param <T> The type of the object to serialize.
     * @return A {@link PersistedData}, if the serialization was successful. Serialization usually fails only because an appropriate type
     *         handler could not be found for the given type.
     */
    ...
    /**
     * Deserializes an object of the given type from a {@link PersistedData} using the stored {@link #persistedDataSerializer} by loading a
     * {@link org.terasology.persistence.typeHandling.TypeHandler TypeHandler} from the {@link #typeHandlerLibrary}.
     *
     ...
     * @return The deserialized object of type {@link T}, if the deserialization was successful. Deserialization usually fails when an
     *         appropriate type handler could not be found for the type {@link T} <i>or</i> if the serialized object representation in
     *         {@code data} does not represent an object of type {@link T}.
     */
    ...
    /**
     * Deserialize an object of type from {@link ByteBuffer}
     *
     * @param type The {@link TypeInfo} specifying the type to deserialize the object as.
     * @param byteBuffer The ByteBuffer contains data for deserialization.
     * @param <T> The type to deserialize the object as.
     * @return an Object if deserialization is success, empty otherwise
     */
    public <T> Optional<T> deserialize(TypeInfo<T> type, ByteBuffer byteBuffer) {
        try {
            D persistedData = reader.read(byteBuffer);
            return deserializeFromPersisted(persistedData, type);
        } catch (IOException e) {
            logger.error("Cannot deserialize type [" + type + "]", e);
        }
     ...
     * @param <T> The type of the object to serialize.
     * @return A ByteArray, if the serialization was successful. Serialization usually fails only because an appropriate type handler could
     *         not be found for the given type.
     */
    ...
    /**
     * Serializes and write the given object to  {@link ByteBuffer}
     *
     * @param object The object to serialize.
     * @param type A {@link TypeInfo} specifying the type of the object to serialize.
     * @param byteBuffer A {@link ByteBuffer} which will used for writing.
     * @param <T> The type of the object to serialize.
     */
    public <T> void serialize(T object, TypeInfo<T> type, ByteBuffer byteBuffer) {
        Optional<D> persistedData = serializeToPersisted(object, type);
        if (persistedData.isPresent()) {
            try {
                writer.writeTo(persistedData.get(), byteBuffer);
            } catch (IOException e) {
                logger.error("Cannot serialize [" + type + "]", e);
            }
        } else {
    ...
    public String toString() {
        return String.format("Serializer with %s", persistedDataSerializer.getClass().getSimpleName());
    }
}
