...
public class CodeHashMap implements CodeMap{
	 ...
	 */
	@Override
	public void insertContent(DrawableCode content, CodeScale scale, CodeMapFactory factory, int x0, int y0) {
		Preconditions.checkArgument(canPlaceContent(content, scale, factory, x0, y0), "Content must be placed in a valida position");
	 ...
	 */
	@Override
	public boolean canPlaceContent(DrawableCode content, CodeScale scale, CodeMapFactory factory, int x, int y) {
		int buildingSize = content.getSize(scale, factory);
	 ...
	 * @param y Coordinate y to be used
	 */
