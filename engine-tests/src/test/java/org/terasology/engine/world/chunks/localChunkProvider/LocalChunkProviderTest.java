...
public class LocalChunkProviderTest {
    ...
    private static void markAllChunksAsReadyExcludingPosition(final ChunkCache chunkCache,
                                                              final Vector3i positionToExclude) {
    ...
    private static void generateMockChunkCubeWithSideWidthAround(final Vector3i position, final int sideWidth,
    ...
    private static void registerBlockWithIdAndEntity(final short blockId, final EntityRef blockEntity,
    ...
    public void testCompleteUpdateMarksChunkReady() throws Exception {
        ...
        final ReadyChunkInfo readyChunkInfo = ReadyChunkInfo.createForNewChunk(chunk, new TShortObjectHashMap<>(),
    ...
    public void testCompleteUpdateRestoresEntitiesForRestoredChunks() throws Exception {
        ...
        final ReadyChunkInfo readyChunkInfo = ReadyChunkInfo.createForRestoredChunk(chunk,
                new TShortObjectHashMap<>(), chunkStore, Collections.emptyList());
        when(chunkFinalizer.completeFinalization()).thenReturn(Collections.singletonList(readyChunkInfo));
    ...
    public void testCompleteUpdateSendsBlockAddedEvents() throws Exception {
        ...
        final ReadyChunkInfo readyChunkInfo = ReadyChunkInfo.createForRestoredChunk(chunk, blockPositionMappings,
                mock(ChunkStore.class), Collections.emptyList());
        when(chunkFinalizer.completeFinalization()).thenReturn(Collections.singletonList(readyChunkInfo));
