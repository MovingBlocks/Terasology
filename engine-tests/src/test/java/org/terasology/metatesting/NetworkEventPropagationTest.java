...
public class NetworkEventPropagationTest {
    ...
    public void testEventPropagation() throws Exception {
        ...
        System.out.println("Sending Broadcast Event from Host...");
        clientEntityOnHost.send(new TestBroadcastEvent("Broadcast"));

        boolean receivedBroadcast = helper.runUntil(2000, () -> clientSystem.receivedBroadcast);
        assertTrue(receivedBroadcast, "Client should receive BroadcastEvent");

        // 2. Test Owner Event (Host -> Client)
        System.out.println("Sending Owner Event from Host...");
        clientEntityOnHost.send(new TestOwnerEvent("Owner"));

        boolean receivedOwner = helper.runUntil(2000, () -> clientSystem.receivedOwner);
        assertTrue(receivedOwner, "Client should receive OwnerEvent");

        // 3. Test Server Event (Client -> Host)
        EntityRef localPlayerEntity = clientContext.get(org.terasology.engine.logic.players.LocalPlayer.class)
                .getClientEntity();
        System.out.println("Sending Server Event from Client...");
        localPlayerEntity.send(new TestServerEvent("Server"));

        boolean receivedServer = helper.runUntil(2000, () -> hostSystem.receivedServer);
        assertTrue(receivedServer, "Host should receive ServerEvent");
    }
