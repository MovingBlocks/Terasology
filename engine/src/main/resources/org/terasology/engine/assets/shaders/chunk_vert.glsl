...
layout (location = 4) in uint in_frames;
...
void main()
{
	...
	if (float(in_frames) > 0) {
	    ...
	    v_uv0.x += frameIndex * TEXTURE_OFFSET;
	    v_uv0.y += floor(v_uv0.x) * TEXTURE_OFFSET;
	    v_uv0.x = mod(v_uv0.x, 1);
    }

	sunVecView = (modelViewMatrix * vec4(sunVec.x, sunVec.y, sunVec.z, 0.0)).xyz;

	isUpside = (in_normal.y > 0.9) ? 1.0 : 0.0;
...
//    gl_FrontColor = gl_Color;
    ...
    if (animated) {
        ...
        if ( checkFlag(BLOCK_HINT_WAVING, blockHint) ) {
           ...
           if (mod(in_uv0.y, TEXTURE_OFFSET) < TEXTURE_OFFSET / 2.0) {
