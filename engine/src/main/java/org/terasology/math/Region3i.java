...
public final class Region3i implements Iterable<Vector3i> {
    ...
    public static Region3i createFromCenterExtents(Vector3fc center, Vector3fc extents) {
        ...
        return createFromMinMax(new Vector3i((int)Math.floor(min.x),(int)Math.floor(min.y),(int)Math.floor(min.z)), new Vector3i((int)Math.ceil(max.x),(int)Math.ceil(max.y),(int)Math.ceil(max.z)));
    }
