...
public final class Region3i implements Iterable<Vector3i> {
    ...
    public static Region3i createFromCenterExtents(Vector3fc center, Vector3fc extents) {
        ...
        return createFromMinMax(new Vector3i((int) min.x,(int)min.y,(int)min.z), new Vector3i((int) max.x,(int)max.y,(int)max.z));
    }
    ...
    public Vector3f center() {
        Vector3f result = new Vector3f().set(min);
        Vector3f halfSize = new Vector3f().set(size);
        halfSize.mul(0.5f);
        result.add(halfSize);
        return result;
    }
