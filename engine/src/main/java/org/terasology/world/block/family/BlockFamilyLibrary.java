...
public class BlockFamilyLibrary {
    ...
    private final ClassLibrary<BlockFamily> library;

    // Used for Inject
    public BlockFamilyLibrary(ModuleManager moduleManager, ReflectFactory reflectFactory,
                              CopyStrategyLibrary copyStrategyLibrary) {
        this(moduleManager.getEnvironment(), reflectFactory, copyStrategyLibrary);
    }

    public BlockFamilyLibrary(ModuleEnvironment moduleEnvironment, ReflectFactory reflectFactory,
                              CopyStrategyLibrary copyStrategyLibrary) {
        library = new DefaultClassLibrary<>(moduleEnvironment, reflectFactory, copyStrategyLibrary);
        for (Class<?> entry : moduleEnvironment.getTypesAnnotatedWith(RegisterBlockFamily.class)) {
            ...
            library.register(new ResourceUrn(moduleEnvironment.getModuleProviding(entry).toString(),
                    registerInfo.value()), (Class<? extends BlockFamily>) entry);
