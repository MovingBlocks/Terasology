...
public class KinematicCharacterMover implements CharacterMover {
    ...
    public CharacterStateEvent step(CharacterStateEvent initial, CharacterMoveInputEvent input, EntityRef entity) {
        ...
        if (worldProvider.isBlockRelevant(initial.getPosition())) {
            ...
            if (input.isFirstRun()) {
                checkBlockEntry(entity, Vector3iUtil.newVector3i(initial.getPosition(), 0.5f), Vector3iUtil.newVector3i(result.getPosition(), 0.5f), characterMovementComponent.height);
            }
    ...
    */
        ...
        if (!newSwimming && !newDiving) { //TODO: generalize to isClimbingAllowed() or similar
            ...
            for (Vector3f side : sides) {
                ...
                if (block.isClimbable()) {
                    ...
                    Vector3i myPos = Vector3iUtil.newVector3i(worldPos, 0.5f);
                    Vector3i climbBlockPos = Vector3iUtil.newVector3i(side, 0.5f);
    ...
    private void climb(final CharacterStateEvent state, CharacterMoveInputEvent input, Vector3f desiredVelocity) {
        ...
        Vector3f climbDir3f = Vector3fUtil.newVector3f(climbDir3i);
