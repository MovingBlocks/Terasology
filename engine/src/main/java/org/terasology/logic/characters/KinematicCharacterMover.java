    ...
    */
    private void checkBlockEntry(EntityRef entity, Vector3i oldPosition, Vector3i newPosition, float characterHeight) {
        ...
        if (!oldPosition.equals(newPosition)) {
            ...
            for (int i = 0; i < characterHeight; i++) {
                ...
                entity.send(new OnEnterBlockEvent(oldBlocks[i], newBlocks[i], JomlUtil.from(new Vector3i(0, i, 0))));
            }
        }
    }
    ...
    private boolean checkStep(CharacterCollider collider, Vector3f position, Vector3f direction, SweepCallback callback,
        ...
        if (!stepped) {
            ...
            boolean moveUpStep = callback.checkForStep(JomlUtil.from(direction), stepHeight, slopeFactor, CHECK_FORWARD_DIST);
    ...
    private void climb(final CharacterStateEvent state, CharacterMoveInputEvent input, Vector3f desiredVelocity) {
        ...
        Quaternionf rotation = JomlUtil.from(new Quat4f(TeraMath.DEG_TO_RAD * state.getYaw(), 0, 0));
...
//        rotation.rotate(tmp, tmp);
        ...
        if (angleToClimbDirection < Math.PI / 4.0 || Math.abs(input.getPitch()) > 60f) {
            if (jumpOrCrouchActive) {
                ...
                rotation = JomlUtil.from(new Quat4f(TeraMath.DEG_TO_RAD * state.getYaw(), TeraMath.DEG_TO_RAD * pitch, 0));
...
//            rotation.rotate(climbDir3f, tmp);
            ...
            if (jumpOrCrouchActive) {
                rotation = JomlUtil.from(new Quat4f(TeraMath.DEG_TO_RAD * state.getYaw(), 0, 0));
            } else {
                rotation =  JomlUtil.from(new Quat4f(TeraMath.DEG_TO_RAD * input.getYaw(), 0f,
