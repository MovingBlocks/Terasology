...
public class ChatSystem extends BaseComponentSystem {
    ...
    @Command(name = "tell2", runOnServer = true, shortDescription = "Sends a private message to a specified user")
    public String tell(EntityRef sender,
                       @CommandParameter(
                               value = "username",
                               suggester = TellCommand.UsernameCommandParameterSuggester.class
                       ) String username,
                       @CommandParameter(
                               value = "message",
                               arrayDelimiter = Command.ARRAY_DELIMITER_VARARGS
                       ) String[] messageArray) {
