...
@Share(Physics.class)
public class BulletPhysicsSystem extends BaseComponentSystem implements UpdateSubscriberSystem, Physics {
    public static final int AABB_SIZE = Integer.MAX_VALUE;

    public static final float SIMD_EPSILON = 1.1920929E-7F;

    private static final Logger logger = LoggerFactory.getLogger(BulletPhysicsSystem.class);
...
//    private List<PhysicsSystem.CollisionPair> collisions = new ArrayList<>();
    ...
    private List<PhysicsTickEvent> delayBuffer = new ArrayList<>();
    private long totalTimeInBuffer = 0;


//    private int totalDelay = 0; // total delay in the playout buffer
    ...
    public void onPhysicsState(PhysicsTickEvent tickEvent, EntityRef entityRef) {
        if(delayBuffer.size() >= 1) {
            PhysicsTickEvent lastTick = delayBuffer.get(delayBuffer.size() - 1);
            totalTimeInBuffer += tickEvent.time - lastTick.time;
        }
        delayBuffer.add(tickEvent);
    }
    ...
    private void newRigidBody(PhysicsTickEvent.CreateBody body) {
        entityRigidBodies.computeIfAbsent(body.target, e -> {
            LocationComponent location = e.getComponent(LocationComponent.class);
            ...
            if (location != null && rigidBody != null && shape != null) {
                ...
                btRigidBody.btRigidBodyConstructionInfo info =
                        new btRigidBody.btRigidBodyConstructionInfo(rigidBody.getMass(),
                ...
                bt.setAngularFactor(body.angularVelocity);
                bt.setLinearFactor(body.linearFactor);
                bt.setFriction(body.friction);
                bt.setLinearVelocity(body.velocity);
                bt.setAngularVelocity(body.angularVelocity);
...
//    }
    ...
    public void update(float delta) {
        accumulator += delta;
        if (delayBuffer.size() >= 2) {
            //ensure that the total time in the buffer is > 100 ms
            while (totalTimeInBuffer > 100 & accumulator > 10.0f) {
                accumulator -= 10.0f;
                totalTimeInBuffer -= 10;

                PhysicsTickEvent event = delayBuffer.get(0);
                if (firstTick) {
                    for (PhysicsTickEvent.CreateBody body : event.toCreate) {
                        newRigidBody(body);
                    }
                    for (EntityRef entity : event.toRemove) {
                        removeRigidBody(entity);
                    }
                    firstTick = false;
                }
                event.time += 10;
                discreteDynamicsWorld.stepSimulation(10.0f, 10);

                PhysicsTickEvent nextTick = delayBuffer.get(1);
                if (nextTick.time - event.time < 0) {
                    delayBuffer.remove(0);
                    firstTick = true;
                }
            }
        }
...
//                    || rigidBody.collidesWith != CollisionGroup.combineGroups(rb.collidesWith)) {
//                removeRigidBody(rigidBody);
//                newRigidBody(entity);
//            } else {
...
//                    CollisionGroup.combineGroups(detectGroups),
...
//
//    private Optional<btPairCachingGhostObject> createTrigger(EntityRef entity) {
//        LocationComponent location = entity.getComponent(LocationComponent.class);
//        TriggerComponent trigger = entity.getComponent(TriggerComponent.class);
//        btCollisionShape shape = getShapeFor(entity);
//        if (shape != null && location != null && trigger != null) {
//            float scale = location.getWorldScale();
//            shape.setLocalScaling(new Vector3f(scale, scale, scale));
//            List<CollisionGroup> detectGroups = Lists.newArrayList(trigge r.detectGroups);
//            CollisionGroup collisionGroup = trigger.collisionGroup;
//
//            Matrix4f startTransform = new Matrix4f().translation(location.getWorldPosition(new Vector3f()));
//            btPairCachingGhostObject tr = new btPairCachingGhostObject();
//            tr.setWorldTransform(startTransform);
//            tr.setCollisionShape(shape);
//            tr.setCollisionFlags(btCollisionObject.CollisionFlags.CF_NO_CONTACT_RESPONSE);
//            tr.userData = entity;
//
////            btPairCachingGhostObject triggerObj = createCollider(
////                    location.getWorldPosition(new Vector3f()),
...
//    }
//
//    private Optional<btRigidBody> createRigidBody(EntityRef entity) {
//        LocationComponent location = entity.getComponent(LocationComponent.class);
...
//            btRigidBody bt = new btRigidBody(info);
//            bt.setAngularFactor(rigidBody.angularFactor);
//            bt.setLinearFactor(rigidBody.linearFactor);
...
////            collider.collidesWith = CollisionGroup.combineGroups(rigidBody.collidesWith);
...
//        insertionQueue.add(new RigidBodyRequest(body, CollisionGroup.combineGroups(groups), CollisionGroup.combineGroups(filter)));
//    }
//
