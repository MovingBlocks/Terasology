...
/**
 * Simple manager for component systems. The manager takes care of registering systems with the Core Registry (if they
 * are marked as shared), initialising them and unloading them. The ComponentSystemManager has two states:
 * <ul>
...
public class ComponentSystemManager {
    ...
    public void loadSystems(ModuleEnvironment environment, NetworkMode netMode) {
        ...
        for (Class<?> type : environment.getTypesAnnotatedWith(RegisterSystem.class)) {
            ...
            if (registerInfo.value().isValidFor(netMode, isHeadless) && areOptionalRequirementsContained(registerInfo
                    , environment)) {
