...
public final class EnvironmentSwitchHandler {
    ...
    private static void registerTypeHandlers(Context context, TypeHandlerLibrary library,
        ...
        for (Class<? extends TypeHandler> handler : environment.getSubtypesOf(TypeHandler.class)) {
            ...
            if (register != null) {
                Optional<Type> opt = GenericsUtil.getTypeParameterBindingForInheritedClass(handler, TypeHandler.class
                        , 0);
                if (opt.isPresent()) {
    ...
    public void handleSwitchToGameEnvironment(Context context) {
        ...
        // typeHandlerLibrary.addTypeHandler(CollisionGroup.class, new CollisionGroupTypeHandler(context.get
        // (CollisionGroupManager.class)));

        TypeHandlerLibrary typeHandlerLibrary = TypeHandlerLibrary.forModuleEnvironment(moduleManager, typeRegistry);
        typeHandlerLibrary.addTypeHandler(CollisionGroup.class,
                new CollisionGroupTypeHandler(context.get(CollisionGroupManager.class)));
    ...
    private <T, U extends CopyStrategy<T>> void registerCopyStrategy(CopyStrategyLibrary copyStrategyLibrary,
                                                                     Class<T> type, Class<U> strategy) {
        try {
    ...
    /**
     * Switches the environment of the asset manager to the specified one. It does not register the prefab formats as
     * they require a proper ComponentLibrary.
     * <p>
     * The existence of this method call is questionable. It has only be introduced to make sure that the asset type
     * manager has never prefab formats that reference an old ComponentLibrary.
     */
    ...
    public void handleSwitchToPreviewEnvironment(Context context, ModuleEnvironment environment) {
        ...
        ComponentLibrary library = new ComponentLibrary(environment, context.get(ReflectFactory.class),
