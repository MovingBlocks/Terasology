...
public class BindsSubsystem implements EngineSubsystem, BindsManager {
    ...
    private final Map<SimpleUri, BindableButton> buttonLookup = Maps.newHashMap();
    private final List<BindableButton> buttonBinds = Lists.newArrayList();
    private Context context;
    private final Map<String, BindableRealAxis> axisLookup = Maps.newHashMap();
    private final List<AbstractBindableAxis> axisBinds = Lists.newArrayList();

    // Links between primitive inputs and bind buttons
    private final Map<Integer, BindableButton> keyBinds = Maps.newHashMap();
    private final Map<MouseInput, BindableButton> mouseButtonBinds = Maps.newHashMap();
    private final Map<ControllerInput, BindableButton> controllerBinds = Maps.newHashMap();
    private final Map<Input, BindableRealAxis> controllerAxisBinds = Maps.newHashMap();
...
//            DirectoryFileSource) {
                ...
                if (result.isSuccess()) {
                    try (ModuleEnvironment environment = moduleManager.loadEnvironment(result.getModules(), false)) {
                        ...
                        Iterable<Class<?>> buttons = environment.getTypesAnnotatedWith(RegisterBindButton.class,
                                filter);
...
//            }
    ...
    private List<Input> fetchDefaultBindings(Class<?> event, BindsConfiguration config) {
        ...
        for (Annotation annotation : event.getAnnotationsByType(DefaultBinding.class)) {
            ...
            if (!boundInputs.contains(defaultInput)) {
                ...
                logger.warn("Input {} is already registered, can not use it as default for event {}", defaultInput,
                        event);
            }
        }
    ...
    private void registerButtonBinds(ModuleEnvironment environment) {
        ...
        for (Class<?> registerBindClass : classes) {
            ...
            if (BindButtonEvent.class.isAssignableFrom(registerBindClass)) {
                try {
                    BindableButton bindButton = registerBindButton(bindUri, info.description(),
                            (BindButtonEvent) registerBindClass.newInstance());
        ...
        for (Class<?> registerBindClass : classes) {
            ...
            if (BindAxisEvent.class.isAssignableFrom(registerBindClass)) {
                ...
                if (positiveButton == null) {
                    logger.warn("Failed to register axis \"{}\", missing positive button \"{}\"", id,
                            info.positiveButton());
                    continue;
                }
                if (negativeButton == null) {
                    logger.warn("Failed to register axis \"{}\", missing negative button \"{}\"", id,
                            info.negativeButton());
                    continue;
                }
                try {
                    BindableAxis bindAxis = registerBindAxis(id.toString(),
                            (BindAxisEvent) registerBindClass.newInstance(), positiveButton, negativeButton);
    ...
    private BindableAxis registerBindAxis(String id, BindAxisEvent event, BindableButton positiveButton,
    ...
    /**
     * Enumerates all active input bindings for a given binding. TODO: Restored for API reasons, may be duplicating code
     * elsewhere. Should be reviewed
     *
     * @param bindId the ID
