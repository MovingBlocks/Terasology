...
/**
 * Manages all input bindings from the {@link ModuleEnvironment}. The manager handles:
 * <ul>
 * <li>Subclasses of {@link BindButtonEvent}, annotated with {@link RegisterBindButton}.</li>
 * <li>Subclasses of {@link BindAxisEvent}, annotated with {@link RegisterBindAxis} or {@link RegisterRealBindAxis}
 * .</li>
 * </ul>
 */
public interface BindsManager {
    ...
    /**
     * The actual binds config. This reflects the current status from {@link #updateDefaultBinds(Context)} and all
     * further modifications like modifications in the user input settings.
     *
     * @return
     */
     ...
     *
     * @return Returns the default bindings. Changes to this config modify the actual instance but become invalid the
     *         next time {@link #updateConfigWithDefaultBinds()} is called.
     */
    BindsConfig getDefaultBindsConfig();

    /**
     * Updates the bindings with their defaults from the current {@link ModuleEnvironment}. After this call {@link
     * #getDefaultBindsConfig()} contains all values for events with {@link RegisterBindButton}, {@link
     * RegisterBindAxis} and {@link RegisterRealBindAxis} which contain one ore more {@link DefaultBinding}.
     * <p>
    ...
    /**
     * Registers all binds from the current {@link ModuleEnvironment}. This contains all events with {@link
     * RegisterBindButton}, {@link RegisterBindAxis} and {@link RegisterRealBindAxis}. {@link
     * #updateConfigWithDefaultBinds()} has to be called before to initialize the config.
     */
    void registerBinds();
    ...
    /**
     * The key bindings. Values map to the {@link KeyId} constants. Changes to the collection or the {@link
     * BindableButton}s modify the system.
     */
    Map<Integer, BindableButton> getKeyBinds();

    /**
     * The axis bindings. Changes to the collection or the {@link AbstractBindableAxis}s modify the system.
     */
