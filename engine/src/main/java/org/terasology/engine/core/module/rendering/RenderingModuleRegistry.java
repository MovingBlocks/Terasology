...
public class RenderingModuleRegistry {
    ...
    /**
     * Fetches list of {@link ModuleRendering} instances from given {@link ModuleEnvironment} and returns ordered list
     * based on their {@code ModuleRendering.initializationPriority} attribute.
     *
    ...
    /**
     * Sorts given list of {@link ModuleRendering} instances by {@code ModuleRendering.initializationPriority}
     * attribute, saves and returns it.
     *
    ...
    private List<ModuleRendering> calculateModuleOrder(Set<ModuleRendering> activeModuleRenderingInstances) {
        orderedModuleRenderingInstances = activeModuleRenderingInstances.stream()
                .sorted(Comparator.comparing(ModuleRendering::getInitPriority)) // sort ascending by 
                // initializationPriority attribute
                .collect(Collectors.toList()); //convert stream to List again
        return orderedModuleRenderingInstances;
    }

    /**
     * Fetches all loaded {@link ModuleRendering} classes from the given {@link ModuleEnvironment} and instantiatest
