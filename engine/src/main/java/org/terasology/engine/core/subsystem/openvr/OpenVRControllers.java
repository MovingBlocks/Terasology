 ...
 * ControllerListener.buttonStateChanged(), OpenVR controller events make it into this class and are stored. Then, by
 * implementing getInputQueue(), an overridden method from ControllerDevice, the Terasology controller system frequently
 * retrieves and handles these events.
 */
class OpenVRControllers implements ControllerDevice, ControllerListener {

    private final Queue<ControllerAction> queuedActions = new ArrayDeque<>();
    ...
    private boolean buttonDown(long buttonIndex) {
        return OpenVRUtil.switchedDown(buttonIndex, cachedStateBefore.ulButtonPressed,
                cachedStateAfter.ulButtonPressed);
    }
