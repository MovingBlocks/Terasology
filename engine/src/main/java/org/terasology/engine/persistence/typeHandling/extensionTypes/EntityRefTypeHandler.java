...
public class EntityRefTypeHandler extends TypeHandler<EntityRef> {
    ...
    public Optional<EntityRef> deserialize(PersistedData data) {
        if (data.isNumber()) {
            ...
            // deserialized yet. See LazyLoadEntityRef for why resolving eagerly here would lose the reference.
            return Optional.of(new LazyLoadEntityRef(entityManager, data.getAsLong()));
        }
