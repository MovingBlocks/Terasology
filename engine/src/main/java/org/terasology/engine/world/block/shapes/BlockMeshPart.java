...
/**
 * Describes the elements composing part of a block mesh. Multiple parts are patched together to define the mesh for a
 * block, or its appearance in the world.
 */
public class BlockMeshPart {
    ...
    public BlockMeshPart mapTexCoords(Vector2f offset, float width, int frames) {
        ...
        for (int i = 0; i < newTexCoords.length; ++i) {
            newTexCoords[i] =
                    new Vector2f(offset.x + normalisedBorder + texCoords[i].x * (width - 2 * normalisedBorder),
    ...
    public void appendTo(ChunkMesh chunk, int offsetX, int offsetY, int offsetZ, ChunkMesh.RenderType renderType,
                         ChunkVertexFlag flags) {
