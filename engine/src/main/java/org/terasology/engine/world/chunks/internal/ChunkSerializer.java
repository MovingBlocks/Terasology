...
public final class ChunkSerializer {
    ...
    public static Chunk decode(EntityData.ChunkStore message, BlockManager blockManager,
        ...
        final TeraArray[] extraData = extraDataManager.makeDataArrays(ChunkConstants.SIZE_X, ChunkConstants.SIZE_Y,
    ...
    private static TeraArray runLengthDecode(EntityData.RunLengthEncoding16 data) {
        Preconditions.checkState(data.getValuesCount() == data.getRunLengthsCount(), "Expected same number of values " +
                "as runs");
        ...
        return new TeraDenseArray16Bit(ChunkConstants.SIZE_X, ChunkConstants.SIZE_Y, ChunkConstants.SIZE_Z,
                decodedData);
    }

    private static TeraArray runLengthDecode(EntityData.RunLengthEncoding8 data) {
        Preconditions.checkState(data.getValues().size() == data.getRunLengthsCount(), "Expected same number of " +
                "values as runs");
    ...
    /**
     * Decode compressed data into an existing TeraArray. Generic w.r.t. TeraArray subclasses, allowing the data to be
     * used for any type of TeraArray.
     */
