...
public class EntityAwareWorldProvider extends AbstractWorldProviderDecorator implements BlockEntityRegistry,
    ...
    @Override
    @SafeVarargs
    public final Block setBlockRetainComponent(Vector3ic position, Block type,
        ...
        if (GameThread.isCurrentThread()) {
            ...
            if (oldType != null) {
                updateBlockEntity(blockEntity, JomlUtil.from(position), oldType, type, false,
                        Sets.newHashSet(components));
            }
            return oldType;
        }
        return null;
    ...
    private boolean isTemporaryBlock(ComponentContainer entity, Block block,
                                     Class<? extends Component> ignoreComponent) {
     ...
     * block type prefabs, but takes into account changes made to the block entity. Components contained in
     * `blockEntity` that
     * <ul>
     ...
     */
    private void updateBlockEntityComponents(EntityRef blockEntity, Block oldType, Block type, Set<Class<?
        ...
        BeforeEntityCreated oldEntityEvent = new BeforeEntityCreated(oldPrefab.orElse(null),
                oldEntityBuilder.iterateComponents());
        ...
        BeforeEntityCreated newEntityEvent = new BeforeEntityCreated(newPrefab.orElse(null),
                newEntityBuilder.iterateComponents());
    ...
    private <T extends Component> void copyIntoPrefab(EntityRef blockEntity, T comp,
    ...
    private <T extends Component> void updateComponent(EntityRef blockEntity, ComponentMetadata<T> metadata,
                                                       T targetComponent) {
