...
/**
 * TeraSparseArray4Bit implements a sparse array with elements of 4 bit size. Its elements are in the range 0 - 15 and
 * it increases memory efficiency by storing two elements per byte. It can further reduce memory consumption through
...
public final class TeraSparseArray4Bit extends TeraSparseArrayByte {
    ...
    public TeraSparseArray4Bit(int sizeX, int sizeY, int sizeZ, byte fill) {
        ...
        Preconditions.checkArgument(fill >= 0 && fill <= 15,
                "Parameter 'fill' has to be in the range 0 - 15 (" + fill + ")");
    }
    ...
    public TeraArray deflate(TeraVisitingDeflator deflator) {
        return Preconditions.checkNotNull(deflator).deflateSparseArray4Bit(inflated, deflated, fill, rowSize(),
