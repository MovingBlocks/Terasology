...
public interface WorldProviderCore {
     ...
     * @return A mapping from world position to previous block type. The value of a map entry is Null if the change
     *         failed (because the necessary chunk was not loaded)
     */
    ...
    /**
     * Sets one of the per-block custom data values at the given position, if it is within the view. You must not use
     * this method with world gen code, call 'setExtraData' on chunk instead.
     *
     * @param index The index of the extra data field
     * @param pos
     * @param value
