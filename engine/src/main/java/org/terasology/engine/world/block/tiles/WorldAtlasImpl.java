...
public class WorldAtlasImpl implements WorldAtlas {
    ...
    private void addTilePart(ResourceUrn uri, int length, List<BlockTile> list, String nameExtension) {
        ...
        if (tile.isPresent()) {
            if (tile.get().getLength() == length) {
                ...
                logger.error("Supplementary tiles {} for tile {} has an incorrect number of frames.", nameExtension,
                        uri);
            }
        }
    ...
    private void buildAtlas() {
        ...
        TextureData terrainTexData = new TextureData(atlasSize, atlasSize, data, Texture.WrapMode.CLAMP,
        ...
        TextureData terrainNormalData = new TextureData(atlasSize, atlasSize, dataNormal, Texture.WrapMode.CLAMP,
                Texture.FilterMode.NEAREST);
        Assets.generateAsset(new ResourceUrn("engine:terrainNormal"), terrainNormalData, Texture.class);

        TextureData terrainHeightData = new TextureData(atlasSize, atlasSize, dataHeight, Texture.WrapMode.CLAMP,
        ...
        for (int i = 0; i < numMipMaps; ++i) {
            ...
            if (i == 0) {
                try (OutputStream stream =
                             new BufferedOutputStream(Files.newOutputStream(PathManager.getInstance().getScreenshotPath().resolve(screenshotName)))) {
    ...
    private void calculateAtlasSizes() {
        tileSize = 16;
        tiles.stream().filter(tile -> tile.getImage(0).getWidth() > tileSize).forEach(tile -> tileSize =
                tile.getImage(0).getWidth());
        ...
        for (int tileIndex = 0; tileIndex < tileImages.size(); tileIndex++) {
            ...
            if (tile == null) {
                ...
                for (int frameIndex = 0; frameIndex < tile.getLength(); frameIndex++) {
                    ...
                    g.drawImage(tile.getImage(frameIndex).getScaledInstance(textureSize, textureSize,
                            Image.SCALE_SMOOTH), posX * textureSize, posY * textureSize, null);
                    totalIndex++;
                }
            }
        }
