...
public abstract class MultiConnectFamily extends AbstractBlockFamily implements UpdatesWithNeighboursFamily {
     ...
     * SideBitFlag.getSides(Side.TOP, Side.BOTTOM) because a column should only connect on the top and bottom. Example
     * 2: In the signalling module, this returns all of the possible sides because a cable can connect in any
     * direction.
     *
     ...
     * @return All of the rotations possible for the block with the given sides
     */
    public Set<Block> registerBlock(BlockUri root, BlockFamilyDefinition definition,
                                    final BlockBuilderHelper blockBuilder, byte sides, Iterable<Rotation> rotations) {
        Set<Block> result = Sets.newLinkedHashSet();
        for (Rotation rotation : rotations) {
            byte sideBits = 0;
            for (Side side : SideBitFlag.getSides(sides)) {
