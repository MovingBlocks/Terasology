 ...
 * the id of a block, and optionally the ResourceUrn of a shape. The final pattern is (see {@link BlockUri} for
 * details): [package]:[blockFamily]:[shapePackage]:[shapeName].[blockIdentifier] the third and forth parts are only
 * used for blocks that don't use the engine:cube shape, and which are generated from a multi-shape block. The
 * blockIdentifier is only used for blocks withing a block family that contains multiple blocks e.g.
 * engine:brickstair.left for left-aligned stairs engine:stone:engine:stair for the family of stone stairs generated
 * from a multishape block definition
 */
