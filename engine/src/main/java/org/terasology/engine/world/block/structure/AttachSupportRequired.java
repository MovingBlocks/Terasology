...
public class AttachSupportRequired implements BlockStructuralSupport {
    ...
    public boolean shouldBeRemovedDueToChange(Vector3i location, Side sideChanged) {
        final AttachSupportRequiredComponent component = getComponent(location,
                Collections.emptyMap());
        if (component != null) {
            final Block block = getBlockWithOverrides(location, Collections.emptyMap());
            return !hasRequiredSupportOnSideForBlock(location, sideChanged, block);
        }
        return false;
    }
    ...
    private boolean hasRequiredSupportOnSideForBlock(Vector3i location, Side sideChanged, Block block) {
        ...
        if (part != null) {
            ...
            return hasSupportFromBlockOnSide(location, sideChanged, Collections.emptyMap());
        }
        return true;
    }
