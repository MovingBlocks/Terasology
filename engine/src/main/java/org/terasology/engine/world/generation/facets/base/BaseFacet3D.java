...
public class BaseFacet3D implements WorldFacet3D {
    ...
    protected final int getRelativeIndex(int x, int y, int z) {
        ...
        return x - relativeRegion.minX() + relativeRegion.getSizeX()
                * (y - relativeRegion.minY() + relativeRegion.getSizeY() * (z - relativeRegion.minZ()));
    }
    ...
    protected final int getWorldIndex(int x, int y, int z) {
        ...
        return x - worldRegion.minX() + worldRegion.getSizeX()
                * (y - worldRegion.minY() + worldRegion.getSizeY() * (z - worldRegion.minZ()));
    }
}
