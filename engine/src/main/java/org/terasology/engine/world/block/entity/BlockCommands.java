...
public class BlockCommands extends BaseComponentSystem {
    ...
    @Command(shortDescription = "List all available blocks\nYou can filter by adding the beginning of words after the" +
            " commands, e.g.: \"listBlocks engine: core\" will list all blocks from the engine and modules starting " +
            "with 'core'",
    ...
    @Command(shortDescription = "Lists all available shapes\nYou can filter by adding the beginning of words after " +
            "the" +
            " commands, e.g.: \"listShapes engine: core\" will list all shapes from the engine and modules starting " +
            "with 'core'",
            requiredPermission = PermissionManager.CHEAT_PERMISSION)
    ...
    public void replaceBlock(
        ...
        if (matchingUris.size() == 1) {
            Optional<BlockFamilyDefinition> def = Assets.get(matchingUris.iterator().next(),
    ...
    /**
     * Called by 'give' command in ItemCommands.java to attempt to put a block in the player's inventory when no item is
     * found. Called by 'giveBulkBlock' command in BlockCommands.java to put a block in the player's inventory.
     *
            ...
            if (def.isPresent()) {
                if (def.get().isFreeform()) {
                    if (shapeUriParam == null) {
                        blockFamily = blockManager.getBlockFamily(new BlockUri(def.get().getUrn(), new ResourceUrn(
                                "engine:cube")));
                    } else {
                        ...
                        blockFamily = blockManager.getBlockFamily(new BlockUri(def.get().getUrn(),
                                resolvedShapeUris.iterator().next()));
                    }
                } else {
                    blockFamily = blockManager.getBlockFamily(new BlockUri(def.get().getUrn()));
                }
