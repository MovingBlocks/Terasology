...
public class BlockManagerImpl extends BlockManager {
    ...
    private final AtomicReference<RegisteredState> registeredBlockInfo = new AtomicReference<>(new RegisteredState());

    private final Set<BlockRegistrationListener> listeners = Sets.newLinkedHashSet();


    private final boolean generateNewIds;
    private final AtomicInteger nextId = new AtomicInteger(1);

    // Cache this for performance reasons because a lookup by BlockURI happens the first time a block is set when 
    // getting the previous block.
    // This causes performance problems eventually down the line when it then uses the ResourceUrn's hashcode to do a
    // lookup into the block map.
                ...
                if (family.isPresent()) {
                    for (Block block : family.get().getBlocks()) {
                        ...
                        if (id != null) {
                            ...
                            logger.error("Missing id for block {} in provided family {}", block.getURI(),
                                    family.get().getURI());
                            if (generateNewIds) {
    ...
    private Optional<BlockFamily> loadFamily(BlockUri uri) {
        Optional<BlockFamilyDefinition> familyDef = assetManager.getAsset(uri.getBlockFamilyDefinitionUrn(),
