...
public abstract class TeraArray {
    ...
    protected TeraArray(int sizeX, int sizeY, int sizeZ, boolean initialize) {
        ...
        checkArgument(getSizeXYZ() % 2 == 0, "The product of the parameters 'sizeX', 'sizeY' and 'sizeZ' has to be a " +
                "multiple of 2 (" + getSizeXYZ() + ")");
        checkArgument(getSizeXZ() % 2 == 0, "The product of the parameters 'sizeX' and 'sizeZ' has to be a multiple " +
                "of 2 (" + getSizeXZ() + ")");
        if (initialize) {
            initialize();
        }
    ...
    /**
     * This is the interface for tera array factories. Every tera array is required to implement a factory. It should be
     * implemented as a static subclass of the corresponding tera array class and it should be called Factory.
     *
    ...
    /**
     * This is the interface for serialization handlers for tera arrays. Every tera array is required to implement a
     * serialization handler. It is recommended to subclass
     * {@link org.terasology.world.chunks.blockdata.TeraArray.BasicSerializationHandler
     * TeraArray.BasicSerializationHandler} instead of using this interface directly. It should be implemented as a
     * static subclass of the corresponding tera array class.
     *
    ...
    /**
     * Extending this class is the recommended way to implement serialization handlers for tera arrays. Tera arrays
     * should implement their serialization handlers as a static subclass called SerializationHandler.
     *
    ...
    protected abstract static class BasicSerializationHandler<T extends TeraArray> implements SerializationHandler<T> {
        ...
        public final ByteBuffer serialize(T array, ByteBuffer toBuffer) {
            ...
            checkArgument(canHandle(array.getClass()),
                    "Unable to handle the supplied array (" + array.getClass().getName() + ")");
