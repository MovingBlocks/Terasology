...
public class BlockBuilder implements BlockBuilderHelper {
    ...
    private final BlockShape cubeShape;
    private final BlockShape lowShape;
    private final BlockShape topShape;
    ...
    @Override
    public Block constructSimpleBlock(BlockFamilyDefinition definition, BlockShape shape, BlockUri uri,
    ...
    @Override
    public Block constructSimpleBlock(BlockFamilyDefinition definition, String section, BlockUri uri,
                                      BlockFamily blockFamily) {
    ...
    @Override
    public Block constructTransformedBlock(BlockFamilyDefinition definition, Rotation rotation, BlockUri uri,
                                           BlockFamily blockFamily) {
        BlockShape shape = definition.getData().getBaseSection().getShape();
        if (shape == null) {
            shape = cubeShape;
    ...
    @Override
    public Block constructTransformedBlock(BlockFamilyDefinition definition, BlockShape shape, String section,
    ...
    @Override
    public Block constructCustomBlock(String defaultName, BlockShape shape, Rotation rotation,
                                      SectionDefinitionData section, BlockUri uri, BlockFamily blockFamily) {
    ...
    private BlockAppearance createAppearance(BlockShape shape, Map<BlockPart, BlockTile> tiles, Rotation rot) {
        ...
        for (BlockPart part : BlockPart.values()) {
            // TODO: Need to be more sensible with the texture atlas. Because things like block particles read from a
            //  part that may not exist, we're being fairly lenient
            Vector2f atlasPos;
            int frameCount;
        ...
        for (Side side : Side.getAllSides()) {
            ...
            if (blockTile != null) {
                BlockMeshPart lowMeshPart = lowShape
                        .getMeshPart(part)
                        .mapTexCoords(JomlUtil.from(worldAtlas.getTexCoords(blockTile, true)),
                                worldAtlas.getRelativeTileSize(), blockTile.getLength());
