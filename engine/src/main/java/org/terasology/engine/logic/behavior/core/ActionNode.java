...
public class ActionNode implements BehaviorNode {
    ...
    public PropertyProvider getProperties() {
        ...
        PropertyProvider provider = new PropertyProvider(CoreRegistry.get(ReflectFactory.class),
                CoreRegistry.get(OneOfProviderFactory.class));
    ...
    public void construct(Actor actor) {
        if (action != null) {
            try {
                ...
                logger.info("Exception while running construct() of action {} from entity {}:", action,
                        actor.getEntity());
            }
        }
    }
