...
public class PlayerSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    ...
    private final List<SpawningClientInfo> clientsPreparingToSpawn = Lists.newArrayList();
    private final List<SpawningClientInfo> clientsPreparingToRespawn = Lists.newArrayList();
    ...
    /**
     * This saves a dead player entity, which is meant to be preserved, to not be destroyed even after the {@link
     * AliveCharacterComponent} is stripped off.
     *
     * @param event
     * @param entity
     ...
     */
    @ReceiveEvent
    public void beforeDestroyDeadPlayer(BeforeDestroyEvent event, EntityRef entity,
    ...
    public void onConnect(ConnectedEvent connected, EntityRef entity) {
        ...
        if (playerStore.hasCharacter()) {
            ...
            Vector3f storedLocation = JomlUtil.from(playerStore.getRelevanceLocation());
    ...
    public void setSpawnLocationOnRespawnRequest(RespawnRequestEvent event, EntityRef entity) {
        ...
        loc.setWorldPosition(JomlUtil.from(spawnPosition));
        loc.setLocalRotation(JomlUtil.from(new Quaternionf()));  // reset rotation
