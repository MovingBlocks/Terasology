...
public class CharacterSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    public static final CollisionGroup[] DEFAULTPHYSICSFILTER = {StandardCollisionGroup.DEFAULT,
    ...
    @ReceiveEvent
    public void beforeDestroy(BeforeDestroyEvent event, EntityRef character, CharacterComponent characterComponent,
        ...
        // TODO: Don't just destroy, ragdoll or create particle effect or something (possible allow another system to
        //  handle)
    ...
    /**
     * Extracts the name from an entity. If the entity is a character, then the display name from the {@link
     * ClientComponent#clientInfo} is used. Otherwise the entity itself is checked for a {@link DisplayNameComponent}.
    ...
    public void onItemUse(OnItemUseEvent event, EntityRef entity,
                          CharacterHeldItemComponent characterHeldItemComponent) {
    ...
    public void onAttackBlock(AttackEvent event, EntityRef entityRef, BlockComponent blockComponent) {
        entityRef.send(new DestroyEvent(event.getInstigator(), event.getDirectCause(),
    ...
    private boolean isPredictionOfEventCorrect(EntityRef character, ActivationRequest event) {
        ...
        if (!(vectorsAreAboutEqual(event.getDirection(), direction))) {
            logger.error("Direction at client {} was different than direction at server {}", event.getDirection(),
                    direction);
        }
        ...
        if (!(vectorsAreAboutEqual(event.getOrigin(), originPos))) {
            String msg = "Player {} seems to have cheated: It stated that it performed an action from {} but the " +
        ...
        if (event.isEventWithTarget()) {
            ...
            HitResult result = physics.rayTrace(originPos, direction, characterComponent.interactionRange,
                    Sets.newHashSet(character), DEFAULTPHYSICSFILTER);
            if (!result.isHit()) {
                String msg = "Denied activation attempt by {} since at the authority there was nothing to activate at" +
                        " that place";
                logger.info(msg, getPlayerNameFromCharacter(character));
                return false;
            }
            ...
            if (!hitEntity.equals(event.getTarget())) {
                 ...
                 * hit test.
                 */
                String msg = "Denied activation attempt by {} since at the authority another entity would have been " +
                        "activated";
                logger.info(msg, getPlayerNameFromCharacter(character));
                return false;
            }
            ...
            if (!(vectorsAreAboutEqual(event.getHitPosition(), result.getHitPoint()))) {
                String msg = "Denied activation attempt by {} since at the authority the object got hit at a differnt" +
                        " position";
                logger.info(msg, getPlayerNameFromCharacter(character));
                return false;
            }
            ...
            if (event.getTarget().exists()) {
                String msg = "Denied activation attempt by {} since the event was not properly labeled as having a " +
                        "target";
                logger.info(msg, getPlayerNameFromCharacter(character));
                return false;
            }
    ...
    public void update(float delta) {
        Iterable<EntityRef> characterEntities = entityManager.getEntitiesWith(CharacterComponent.class,
    ...
    private boolean isDistanceToLarge(LocationComponent characterLocation, LocationComponent targetLocation,
    ...
    @ReceiveEvent
    public void onScaleCharacter(OnScaleEvent event, EntityRef entity, CharacterComponent character,
