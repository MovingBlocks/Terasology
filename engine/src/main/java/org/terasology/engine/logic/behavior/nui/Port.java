...
/**
 * Represents a port at a RenderableNode. There are several types of ports: - InputPort (one per RenderableNode) -
 * OutputPort (unlimited per RenderableNode, may be restricted by the type of the node) - InsertPort ("virtual" port, to
 * allow connections placed between two existing ones)
 * <br><br>
 * Input/Output ports may have a target. This is always of the opposite type. When setting a target to a port, the node
 * of the InputPort is added to the child list of the node of the OutputPort.
 */
public abstract class Port extends CoreWidget {
    protected RenderableNode node;
    protected Rect2f rect;
    private final UITextureRegion active = Assets.getTextureRegion("engine:checkboxChecked").get();
    private final UITextureRegion inactive = Assets.getTextureRegion("engine:checkbox").get();

    private final InteractionListener connectListener = new BaseInteractionListener() {
        @Override
