...
/**
 * System to allow the use of BlockSelectionComponents. This system is a client only system, though no other player will
 * see selections done by one player.  This system uses an item's activate event to both start and end a selection, then
 * triggers a ApplyBlockSelectionEvent event.
 */
...
public class LocalPlayerBlockSelectionByItemSystem extends BaseComponentSystem {
    ...
    public void onPlaced(ActivateEvent event, EntityRef itemEntity) {
        ...
        if (null == blockSelectionComponent.startPosition) {
            ...
            localPlayer.getCharacterEntity().send(new ApplyBlockSelectionEvent(itemEntity,
    ...
    public void onCamTargetChanged(CameraTargetChangedEvent event, EntityRef entity) {
        ...
        BlockSelectionComponent blockSelectionComponent =
                blockSelectionComponentEntity.getComponent(BlockSelectionComponent.class);
        ...
        if (blockSelectionComponent.isMovable) {
            ...
            blockSelectionComponent.currentSelection = Region3i.createFromCenterExtents(new Vector3i(targetLocation.x
    ...
    /**
     * This event is sent after the size of a region is finalized and the location is to yet to be decided by the
     * player. This event marks the start of the camera position binding with the region.
     *
     * @param event The event sent
     * @param blockSelectionComponentEntity The entity sending the event. This entity must have the {@link
     *         BlockSelectionComponent}
     */
    @ReceiveEvent(components = {BlockSelectionComponent.class})
    public void onMovableBlockSelectionStart(MovableSelectionStartEvent event,
                                             EntityRef blockSelectionComponentEntity) {
