 ...
 * <p>
 * Entity Creation Flow: - ClientComponent for the local player added or changed - If Camera does not exist, create
 * "engine:camera" entity - Save the ClientComponent with the new camera attached
 * <p>
 * Reset Camera Event Flow: - ResetCameraEvent - Camera entity on the ClientComponent is destroyed and set to
 * EntityRef.NULL - ClientComponent is saved
 * <p>
 * Auto Mount Camera Flow: - ClientComponent or AutoMountCameraComponent for the local player added or changed - The
 * local player's camera entity (created from above steps) is location linked to the Gaze Entity
 */
...
public class CameraClientSystem extends BaseComponentSystem {
    ...
    @ReceiveEvent
    public void ensureCameraEntityCreatedOnChangedClientComponent(OnChangedComponent event, EntityRef client,
    ...
    @ReceiveEvent
    public void ensureCameraEntityCreatedOnActivateClientComponent(OnActivatedComponent event, EntityRef client,
                                                                   ClientComponent clientComponent) {
        if (localPlayer.getClientEntity().equals(client)) {
            ensureCameraEntityCreated();
        }
    ...
    @ReceiveEvent
    public void resetCameraForClient(ResetCameraEvent resetCameraEvent, EntityRef entityRef,
                                     ClientComponent clientComponent) {
    ...
    public void mountCameraOnActivate(OnActivatedComponent event, EntityRef entityRef,
                                      AutoMountCameraComponent autoMountCameraComponent,
                                      ClientComponent clientComponent) {
    ...
    private void mountCamera() {
        ...
        if (cameraLocation != null) {
            Location.attachChild(targetEntityForCamera, clientComponent.camera, cameraLocation.getLocalPosition(),
                    new Quat4f(Quat4f.IDENTITY));
        } else {
            Location.attachChild(targetEntityForCamera, clientComponent.camera, Vector3f.zero(),
                    new Quat4f(Quat4f.IDENTITY));
        }
    }
}
