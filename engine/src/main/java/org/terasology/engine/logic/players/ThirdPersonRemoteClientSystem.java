...
public class ThirdPersonRemoteClientSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
        ...
        for (EntityRef remotePlayer : entityManager.getEntitiesWith(CharacterComponent.class,
            ...
            while (heldItermsIterator.hasNext()) {
                ...
                if (itemIsRemotelyHeldComponent.remotePlayer.equals(remotePlayer)) {
                    logger.debug("Looks like we got a match for an existing item! Removing it from the set and " +
                            "processing it");
                    currentHeldItem = heldItemCandidate;
            ...
            // selected
            if (currentHeldItem == EntityRef.NULL) {
                logger.debug("During update we hit a player without a current held item entity. Checking {} to see if" +
                        " we should make one", remotePlayer);
                if (remotePlayer.hasComponent(CharacterHeldItemComponent.class)) {
                    ...
                    if (characterHeldItemComponent != null && !characterHeldItemComponent.selectedItem.equals(EntityRef.NULL)) {
                        logger.debug("Yep that player holds an item. Calling linkHeldItemLocationForRemotePlayer with" +
                                " item {}", characterHeldItemComponent.selectedItem);
    ...
    private boolean relatesToLocalPlayer(EntityRef entity) {
        if (entity == null || entity.equals(EntityRef.NULL)) {
            logger.debug("checkForLocalPlayer given a bad entity (null or NullEntityRef - can't relate that to a " +
                    "local player!");
            return false;
        }
        ...
        if (entity.hasComponent(CharacterComponent.class)) {
            ...
            if (controller != null) {
                if (controller.equals(localPlayer.getClientEntity())) {
                    logger.debug("checkForLocalPlayer found its entity to be a character whose controller matched the" +
                            " localPlayer client entity! {}", controller);
                    return true;
                }

                if (controller.equals(networkSystemProvidedClientEntity)) {
                    logger.debug("checkForLocalPlayer found its entity to be a character controller matching the " +
                            "network system provided local client entity! {}", controller);
                    return true;
                }
            }
        }
