...
/**
 * Calculates character movement using a physics-engine provided CharacterCollider. This collider is swept through the
 * world to detect collisions. The general process for this is:
 * <ol>
...
public class KinematicCharacterMover implements CharacterMover {
     ...
     * @param newClimbing True if the character has a climbable block near him and is in conditions to climb it
     *         (not swimming or diving).
     */
    static void updateMode(CharacterStateEvent state, boolean newSwimming, boolean newDiving, boolean newClimbing,
        ...
        if (newDiving) {
            ...
            state.getVelocity().y = 0;
            state.setMode((newClimbing) ? MovementMode.CLIMBING : isCrouching ? MovementMode.CROUCHING :
    ...
    private void checkBlockEntry(EntityRef entity, Vector3i oldPosition, Vector3i newPosition, float characterHeight) {
        ...
        //          private void checkBlockEntry(EntityRef entity, Vector3i oldPosition, Vector3i newPosition, 
        //          Vector3f characterExtents) {
    ...
    /**
     * Checks whether a character should change movement mode (from being underwater or in a ladder). A higher and lower
     * point of the character is tested for being in water, only if both points are in water does the character count as
     * swimming.
     * <br><br>
    ...
    private Vector3i findClimbable(CharacterMovementComponent movementComp, Vector3f worldPos, boolean swimming,
                                   boolean diving) {
    ...
    /**
     * Checks of the player will step up to an object. In a single movement step the player can only step up a single
     * item.
     *
     * @param collider
     * @param position
    ...
    private boolean checkStep(CharacterCollider collider, Vector3f position, Vector3f direction, SweepCallback callback,
        ...
        if (!stepped) {
            ...
            boolean moveUpStep = callback.checkForStep(JomlUtil.from(direction), stepHeight, slopeFactor,
                    CHECK_FORWARD_DIST);
    ...
    private Vector3f extractResidualMovement(Vector3f hitNormal, Vector3f direction, float normalMag) {
        ...
        if (movementLength > physics.getEpsilon()) {
            ...
            Vector3f perpendicularDir =
                    hitNormal.mul(reflectDir.dot(hitNormal), new Vector3f()).mul(-1).add(reflectDir);
            if (normalMag != 0.0f) {
    ...
    private MoveResult move(final Vector3f startPosition, final Vector3f moveDelta, final float stepHeight,
        ...
        hitSide = moveHorizontal(new Vector3f(moveDelta.x, 0, moveDelta.z), collider, position, slopeFactor,
                stepHeight);
    ...
    private boolean moveDown(float dist, float slopeFactor, CharacterCollider collider, Vector3f position) {
        ...
        while (remainingDist > physics.getEpsilon() && iteration++ < 10) {
            SweepCallback callback = collider.sweep(JomlUtil.from(position), JomlUtil.from(targetPos),
                    VERTICAL_PENETRATION, -1.0f);
            ...
            if (callback.hasHit()) {
                float originalSlope = callback.getHitNormalWorld().dot(JomlUtil.from(new Vector3f(0, 1, 0)));
                if (originalSlope < slopeFactor) {
                    ...
                    if (slope < slopeFactor) {
                        ...
                        expectedMove.sub(position);
                        extractResidualMovement(JomlUtil.from(callback.getHitNormalWorld()), expectedMove);
    ...
    private boolean moveHorizontal(Vector3f horizMove, CharacterCollider collider, Vector3f position, float slopeFactor,
        ...
        while (remainingFraction >= 0.01f && iteration++ < 10) {
            SweepCallback callback = collider.sweep(JomlUtil.from(position), JomlUtil.from(targetPos),
                    HORIZONTAL_PENETRATION, slopeFactor);

            /* Note: this isn't quite correct (after the first iteration the closestHitFraction is only for part of 
            the moment)
            ...
            if (callback.hasHit()) {
                ...
                newDir.mul(dist);
                float slope = callback.getHitNormalWorld().dot(JomlUtil.from(new Vector3f(0, 1, 0)));
                ...
                if (slope < slopeFactor || 1 - slope < physics.getEpsilon()) {
                    ...
                    Vector3f newHorizDir = new Vector3f(newDir.x, 0, newDir.z);
                    extractResidualMovement(JomlUtil.from(callback.getHitNormalWorld()), newDir);
    ...
    private float moveUp(float riseAmount, CharacterCollider collider, Vector3f position) {
        ...
        if (collider != null) {
            SweepCallback callback = collider.sweep(JomlUtil.from(position), JomlUtil.from(to),
                    VERTICAL_PENETRATION_LEEWAY, -1f);
            if (callback.hasHit()) {
                float actualDist = Math.max(0,
    ...
    private void walk(final CharacterMovementComponent movementComp, final CharacterStateEvent state,
        ...
        if (movementComp.mode.scaleGravity == 0) {
            ...
            endVelocity.y += Math.max(-TERMINAL_VELOCITY,
                    velocityDiff.y - (GRAVITY * movementComp.mode.scaleGravity) * input.getDelta());
        } else {
            endVelocity.y = Math.max(-TERMINAL_VELOCITY,
                    state.getVelocity().y - (GRAVITY * movementComp.mode.scaleGravity) * input.getDelta());
        }
        ...
        MoveResult moveResult = move(JomlUtil.from(state.getPosition()), moveDelta,
                (state.getMode() != MovementMode.CLIMBING && state.isGrounded() && movementComp.mode.canBeGrounded) ?
                        movementComp.stepHeight : 0,
        ...
        if (input.isFirstRun() && distanceMoved.length() > 0) {
            entity.send(new MovedEvent(new ImmutableVector3f(JomlUtil.from(distanceMoved)),
                    new ImmutableVector3f(state.getPosition())));
        }
    ...
    public static class MoveResult {
        ...
        private final Vector3f finalPosition;
        private final boolean horizontalHit;
        private final boolean bottomHit;
