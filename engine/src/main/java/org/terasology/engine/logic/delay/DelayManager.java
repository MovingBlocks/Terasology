 ...
 * action on the same entity. Therefore, it is crucial that actionIds are unique to the specific operation you want to
 * perform on the entity.
 * <p>
 * Periodic action allows you to schedule an event being sent to this entity periodically at the specified interval. The
 * event sent for periodic action to this entity is PeriodicActionTriggeredEvent. Upon receiving this event the method
 * should check that it has the expected actionId, as multiple different systems might register periodic action on the
 * same entity. Therefore, it is crucial that actionIds are unique to the specific operation you want to perform on the
 * entity.
 * <p>
