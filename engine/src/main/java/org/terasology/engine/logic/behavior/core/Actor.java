 ...
 * <br><br>
 * Besides the actual entity, a blackboard is stored for each actor. Every node may read or write to this blackboard, to
 * communicate their states or exchange variables with other nodes.
 */
@API
public class Actor {
    ...
    public Object getComponentField(ComponentFieldUri uri) {
        ...
        ComponentMetadata<? extends Component> metadata =
                componentLibrary.getMetadata(new ResourceUrn(uri.getComponentUri().toString()));
        if (metadata == null) {
            return null;
        }
