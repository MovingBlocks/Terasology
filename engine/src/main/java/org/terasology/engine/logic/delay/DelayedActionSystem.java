...
public class DelayedActionSystem extends BaseComponentSystem implements UpdateSubscriberSystem, DelayManager {
    ...
    private final SortedSetMultimap<Long, EntityRef> delayedOperationsSortedByTime = TreeMultimap.create(Ordering.natural()
            , Ordering.arbitrary());
    private final SortedSetMultimap<Long, EntityRef> periodicOperationsSortedByTime =
    ...
    private void invokePeriodicOperations(long currentWorldTime) {
        ...
        operationsToInvoke.stream().filter(EntityRef::exists).forEach(periodicEntity -> {
            final PeriodicActionComponent periodicActionComponent =
                    periodicEntity.getComponent(PeriodicActionComponent.class);
            ...
            if (periodicActionComponent != null) {
                final Set<String> actionIds =
                        periodicActionComponent.getTriggeredActionsAndReschedule(currentWorldTime);
    ...
    @ReceiveEvent
    public void delayedComponentActivated(OnActivatedComponent event, EntityRef entity,
                                          DelayedActionComponent delayedActionComponent) {
        delayedOperationsSortedByTime.put(delayedActionComponent.getLowestWakeUp(), entity);
    }

    @ReceiveEvent
    public void periodicComponentActivated(OnActivatedComponent event, EntityRef entity,
    ...
    @ReceiveEvent
    public void delayedComponentDeactivated(BeforeDeactivateComponent event, EntityRef entity,
                                            DelayedActionComponent delayedActionComponent) {
        delayedOperationsSortedByTime.remove(delayedActionComponent.getLowestWakeUp(), entity);
    }

    @ReceiveEvent
    public void periodicComponentDeactivated(BeforeDeactivateComponent event, EntityRef entity,
                                             PeriodicActionComponent periodicActionComponent) {
    ...
    public void addDelayedAction(EntityRef entity, String actionId, long delay) {
        ...
        if (delayedActionComponent != null) {
            ...
            if (oldWakeUp < newWakeUp) {
                ...
                // Even if the oldWakeUp time is greater than or equal to the new one, the next action should still 
                // be added
