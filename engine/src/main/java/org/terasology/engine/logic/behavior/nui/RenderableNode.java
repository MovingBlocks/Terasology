 ...
 */
public class RenderableNode extends CoreWidget implements ZoomableLayout.PositionalWidget<BehaviorEditor>,
        TreeAccessor<RenderableNode> {
    private final List<RenderableNode> children = Lists.newArrayList();
    private final UITextureRegion texture = Assets.getTextureRegion("engine:button").get();
    private final PortList portList;

    private BehaviorNode node;
    private Vector2f position;
    private Vector2f size;
    private final TreeAccessor<RenderableNode> withoutModel;
    private final TreeAccessor<RenderableNode> withModel;
    private final BehaviorNodeComponent data;
    ...
    private final InteractionListener moveListener = new BaseInteractionListener() {
        @Override
        ...
        public boolean onMouseClick(NUIMouseClickEvent event) {
            ...
            dragged = false;
            copyMode =
                    button == MouseInput.MOUSE_LEFT && (keyboard.isKeyDown(Keyboard.KeyId.LEFT_SHIFT) || keyboard.isKeyDown(Keyboard.KeyId.RIGHT_SHIFT));
            if (copyMode) {
