...
/**
 * Call {@link #initialize(Context)} before using this widget. (Ideally the logic would be moved to the
 * BehaviorEditorScreen instead)
 */
public class BehaviorEditor extends ZoomableLayout implements DefaultBehaviorTreeRunner.Callback {
    ...
    private final Map<BehaviorNode, BehaviorState> stateMap = Maps.newHashMap();
            ...
            for (UIWidget widget : getWidgets()) {
                ...
                if (widget instanceof RenderableNode) {
                    ...
                    for (Port port : renderableNode.getPorts()) {
                        ...
                        drawConnection(canvas, port, targetPort, port == activeConnectionStart ? Color.BLACK :
                                Color.GREY);
                    }
                }
            }
    ...
    public void layout(RenderableNode start) {
        ...
        TreeLayout<RenderableNode> layout = new TreeLayout<>(layoutTree,
                new FixedNodeExtentProvider<RenderableNode>(10, 5), new DefaultConfiguration<RenderableNode>(4, 2));
