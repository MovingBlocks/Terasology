...
/**
 * This system uses the pickup prefab on an item component to add on extra components needed for the item to display in
 * the world. The default set of components will add a location, lifetime, and rigid body.  This will allow the item to
 * be seen (pending it has a mesh), to interact with gravity, and to disappear after a while.
 */
