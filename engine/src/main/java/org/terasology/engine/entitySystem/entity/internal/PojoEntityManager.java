...
public class PojoEntityManager implements EngineEntityManager {
    ...
    private final TLongSet loadedIds = new TLongHashSet();

    private final EngineEntityPool globalPool = new PojoEntityPool(this);
    private final PojoSectorManager sectorManager = new PojoSectorManager(this);
    private final Map<Long, EngineEntityPool> poolMap = new MapMaker().initialCapacity(1000).makeMap();
    private final List<EngineEntityPool> worldPools = Lists.newArrayList();
    private final Map<EngineEntityPool, Long> poolCounts = new HashMap<EngineEntityPool, Long>();

    private final Set<EntityChangeSubscriber> subscribers = Sets.newLinkedHashSet();
    private final Set<EntityDestroySubscriber> destroySubscribers = Sets.newLinkedHashSet();
