...
public final class ChunkMath {
     ...
     *         {@link #calcChunkPos(Vector3ic, Vector3ic, org.joml.Vector3i)}.
     */
    @Deprecated
     ...
     * @return
     * @deprecated This is scheduled for removal in an upcoming version method will be replaced with JOML implementation
     *         {@link #calcChunkPos(Vector3fc, org.joml.Vector3i)}.
     */
    @Deprecated
     ...
     * @return
     * @deprecated This is scheduled for removal in an upcoming version method will be replaced with JOML implementation
     *         {@link #calcChunkPos(Vector3ic, org.joml.Vector3i)}.
     */
    @Deprecated
     ...
     * @return
     * @deprecated This is scheduled for removal in an upcoming version method will be replaced with JOML implementation
     *         {@link #calcChunkPos(int, int, int, org.joml.Vector3i)}.
     */
    @Deprecated
    ...
    public static Vector3i calcChunkPos(int x, int y, int z, Vector3i chunkPower) {
        return new Vector3i(calcChunkPos(x, chunkPower.x), calcChunkPos(y, chunkPower.y), calcChunkPos(z,
                chunkPower.z));
    }
    ...
    public static org.joml.Vector3i calcChunkPos(Vector3fc pos, org.joml.Vector3i dest) {
        return calcChunkPos(pos.x(), pos.y(), pos.z(), ChunkConstants.POWER_X, ChunkConstants.POWER_Y,
                ChunkConstants.POWER_Z, dest);
    }
    ...
    /**
     * The position of the chunk given the coordinate and size of chunk in powers of 2. This uses the default power
     * ({@link ChunkConstants#POWER_X}, {@link ChunkConstants#POWER_Y}, {@link ChunkConstants#POWER_Z})
     *
     * <p>default chunk size ({@link ChunkConstants#SIZE_X}, {@link ChunkConstants#SIZE_Y}, {@link
     * ChunkConstants#SIZE_Z}) </p>
     *
     ...
     * @return dest
     */
    public static org.joml.Vector3i calcChunkPos(float x, float y, float z, int chunkX, int chunkY, int chunkZ,
                                                 org.joml.Vector3i dest) {
        return calcChunkPos(Math.roundUsing(x, org.joml.RoundingMode.FLOOR), Math.roundUsing(y,
                org.joml.RoundingMode.FLOOR), Math.roundUsing(z, org.joml.RoundingMode.FLOOR), chunkX, chunkY, chunkZ
                , dest);
    }
     ...
     * @return dest
     */
    public static org.joml.Vector3i calcChunkPos(int x, int y, int z, int chunkX, int chunkY, int chunkZ,
                                                 org.joml.Vector3i dest) {
     ...
     * @return dest
     */
    public static BlockRegion calcChunkRegion(BlockRegion region, int chunkX, int chunkY, int chunkZ,
                                              BlockRegion dest) {
        return dest.
                setMin(calcChunkPos(region.getMinX(), chunkX), calcChunkPos(region.getMinY(), chunkY),
                        calcChunkPos(region.getMinZ(), chunkZ)).
                setMax(calcChunkPos(region.getMaxX(), chunkX), calcChunkPos(region.getMaxY(), chunkY),
                        calcChunkPos(region.getMaxZ(), chunkZ));
    }
    ...
    /**
     * calculates a region that encasing a chunk This uses the default power ({@link ChunkConstants#POWER_X}, {@link
     * ChunkConstants#POWER_Y}, {@link ChunkConstants#POWER_Z})
     *
     * @param region a bounding box that is contained
     * @param dest will hold the result
     * @return dest
     ...
     * @return
     * @deprecated This is scheduled for removal in an upcoming version method will be replaced with JOML implementation
     *         {@link #calcChunkRegion(BlockRegion, BlockRegion)}.
     */
     ...
     * @return
     * @deprecated This is scheduled for removal in an upcoming version method will be replaced with JOML implementation
     *         {@link #calcChunkRegion(BlockRegion, Vector3ic, BlockRegion)}.
     */
    ...
    public static Vector3i calcRelativeBlockPos(int x, int y, int z, Vector3i chunkFilterSize) {
        return new Vector3i(calcRelativeBlockPos(x, chunkFilterSize.x), calcRelativeBlockPos(y, chunkFilterSize.y),
                calcRelativeBlockPos(z, chunkFilterSize.z));
    }
    ...
    public static void populateMinAdjacent2D(int[] source, int[] target, int dimX, int dimY, boolean populateMargins) {
        ...
        if (populateMargins) {
            ...
            for (int y = 1; y < dimY - 1; ++y) {
                // x == 0
                target[y * dimX] = Math.min(source[dimX * (y - 1)], Math.min(source[dimX * (y + 1)],
                        source[1 + dimX * y]));
                // x == dimX - 1
            ...
            // x == dimX - 1; y == dimY - 1
            target[dimX - 1 + dimX * (dimY - 1)] = Math.min(source[dimX - 2 + dimX * (dimY - 1)],
                    source[dimX - 1 + dimX * (dimY - 2)]);
        }
    }
