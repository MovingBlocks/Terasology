...
public class AudioSystem extends BaseComponentSystem implements UpdateSubscriberSystem {
    ...
    public void playTestSound(@Sender EntityRef sender, @CommandParam("xOffset") float xOffset, @CommandParam(
    ...
    public void onPlaySound(PlaySoundEvent playSoundEvent, EntityRef entity) {
        ...
        if (location != null && !Float.isNaN(location.getWorldPosition().x)) {
            audioManager.playSound(playSoundEvent.getSound(), location.getWorldPosition(), playSoundEvent.getVolume()
                    , AudioManager.PRIORITY_NORMAL);
        } else {
    ...
    public void update(float delta) {
        audioManager.updateListener(localPlayer.getPosition(), localPlayer.getViewRotation(),
