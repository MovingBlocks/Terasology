...
public class LwjglCanvasRenderer implements CanvasRenderer, PropertyChangeListener {
    ...
    public void drawMesh(Mesh mesh, Material material, Rect2i drawRegion, Rect2i cropRegion, Quaternionf rotation, Vector3f offset, float scale, float alpha) {
        ...
        Matrix4f centerTransform = new Matrix4f().translationRotateScale(centerOffset,new Quaternionf().identity(),1);//BaseQuat4f.IDENTITY, centerOffset, 1.0f);
        Matrix4f userTransform = new Matrix4f().translationRotateScale(offset,rotation,-fitScale * scale); //new Matrix4f(rotation, offset, -fitScale * scale);
        Matrix4f translateTransform =  new Matrix4f().translationRotateScale(
                new Vector3f((drawRegion.minX() + drawRegion.width() / 2) * uiScale,
                    (drawRegion.minY() + drawRegion.height() / 2) * uiScale, 0),new Quaternionf(), 1);
