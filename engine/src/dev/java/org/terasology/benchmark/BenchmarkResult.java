...
/**
 * BenchmarkResult records the results and the errors of the execution of one particular benchmark. It also maintains a
 * list of columns which are very useful for pretty printing the results.
 */
public abstract class BenchmarkResult {
    ...
    public BenchmarkResult(Benchmark benchmark) {
        ...
        Preconditions.checkArgument(repetitions.length > 0, "The length of the parameter 'repetitions' has to be " +
                "greater than zero");
    ...
    public abstract static class Column<T extends BenchmarkResult> {
        ...
        public final String getValue(int rep) {
            ...
            if (cache[rep] == null) {
                String v = Preconditions.checkNotNull(getValueInternal(rep), "BenchmarkResult::Column" +
                        "::getValueInternal() must never return null (rep=" + rep + ")");
                cache[rep] = v;
                return v;
            }
            return cache[rep];
