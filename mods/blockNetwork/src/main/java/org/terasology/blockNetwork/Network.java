 ...
 * 3. A network can have no networking nodes at all, and up to two leaf nodes (neighbouring leaf blocks).
 *
...
public class Network {
    ...
    public boolean removeLeafBlock(Vector3i location) {
        ...
        final Byte removedDirections = leafPossibleConnections.remove(location);
        if (removedDirections == null)
            throw new IllegalStateException("Tried to remove a block that is not in the network");
        leafActualConnections.remove(location);

        if (isDegeneratedNetwork())
            leafActualConnections.clear();
    ...
    public Collection<Network> removeNetworkingBlock(Vector3i location) {
        ...
        for (Map.Entry<Vector3i, Byte> leafBlock : leafPossibleConnections.entrySet()) {
            ...
            if (!addedToNetwork) {
                Network newNetwork = new Network();
                newNetwork.addLeafBlock(leafBlockLocation, leafBlockDirections);
                resultNetworks.add(newNetwork);
            }
        }
