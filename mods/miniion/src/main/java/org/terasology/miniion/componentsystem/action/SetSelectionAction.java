...
public class SetSelectionAction implements EventHandlerSystem {
	...
	public void onActivate(ActivateEvent event, EntityRef entity) {
		ZoneSelectionComponent comp = entity
		...
		if (gridPositions.size() == 0) {
			...
			comp.blockGrid.addGridPosition(comp.startpos, worldProvider
					.getBlock(comp.startpos).getId());
		} else if (gridPositions.size() == 1) {
			comp.endpos = new Vector3i(event.getTargetLocation());
			comp.blockGrid.addGridPosition(comp.endpos,
					worldProvider.getBlock(comp.endpos).getId());
				...
				for (int y = minbounds.y; y < maxbounds.y; y++) {
					for (int z = minbounds.z; z < maxbounds.z; z++) {
						...
						if (!tmpblock.isInvisible()) {
							grid.addGridPosition(new Vector3i(x, y, z),
									tmpblock.getId());
						}
					}
				}
