...
public class UIMinionbar extends UIDisplayContainer {
	...
	private class UIMinionbarCell extends UIDisplayContainer {
		...
		public void render() {
			...
			MinionBarComponent inventory = CoreRegistry.get(LocalPlayer.class)
					.getEntity().getComponent(MinionBarComponent.class);
			if (inventory == null) {
				return;
			}
			if (inventory.minionSlots.size() <= id) {
			...
			MinionComponent minion = minionEntity
	...
	public UIMinionbar() {
		...
		// CoreRegistry.get(LocalPlayer.class).getEntity().getComponent(MinionControllerComponent.class);
	 ...
	 * MinionControllerComponent.class}) public void
	 * onMinionChanged(MinionChangedEvent event, EntityRef entity) {
	 * MinionControllerComponent minionController =
	 * entity.getComponent(MinionControllerComponent.class); if
	 * (minionController.selectedMinion != prevSelected) {
	 * cells[minionController.selectedMinion].setSelected(true);
	 * cells[prevSelected].setSelected(false); prevSelected =
